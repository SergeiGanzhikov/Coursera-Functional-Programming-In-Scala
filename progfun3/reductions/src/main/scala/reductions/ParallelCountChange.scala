...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    def loop(money: Int, coins: List[Int], accumulator: Int): Int = {
      if (money == 0) accumulator + 1
      else if (money < 0 || coins.isEmpty)  accumulator
      else loop(money - coins.head, coins, accumulator + countChange(money, coins.tail))
    }
    loop(money, coins, 0)
  }
